...
class TestBookDaoWithPropertyTypes extends WordSpec with BeforeAndAfter with MustMatchers {
  ...
  val bookDao = new BookDaoWithPropertyTypes(TransactionManager(ds))
  ...
  "The persist method" when {
    ...
    "invoked with a persistent book" must {
      "throw an exception" in {
        ...
        an[Exception] must be thrownBy (bookDao.persist(b1))
      }
    }
  }
  "The update method" when {
    ...
    "invoked with a transient book" must {
      "throw an exception" in {
        ...
        an[Exception] must be thrownBy (bookDao.update(b1))
      }
    }
  }
